...
For any $w \in W$, there is a unique nilpotent orbit $\mathrm{St}(w)$ such that $\mu(\overline{T^*_{\mathcal{Y}_w}(\mathcal{B} \times \mathcal{B})}) = \overline{\mathrm{St}(w)}$. The map $w \mapsto \mathrm{St}(w)$ defines a \textcolor{AttentionColor}{surjective(?)} map from $W$ to the set of nilpotent orbits in $\fg^*$ (this map can be computed via Robinson-Schensted algorithm).
...
\[
  \BR p_{\CN,*}\underline{\BQ}_{\widetilde{\CN}} \simeq \BR p_{*}\underline{\BQ}_{\widetilde{\fg}})|_{\CN}. \quad  \textcolor{AttentionColor}{(\mathrm{Spr} = \BR p_{\CN,*}\underline{\BQ}_{\widetilde{\CN}}[\dim \CN] \ called \ The \ Springer \ sheaf)}
\]
...
\section{2025.10.29 Unipotent Arthur packets and special unipotent representation \texorpdfstring{\cite{ABV92}}{[ABV92]}}
...
\begin{itemize}
  \item \textcolor{AttentionColor}{Problem: How to compute the $j$-induction explicitly?}
  \item \textcolor{AttentionColor}{Problem: the special repn of each cell always appears?}
\end{itemize}
...
\begin{itemize}
  ...
  \item By computation, prove that in the case of metaplectic groups, metaplectic quasi-admissible equivalent to metaplectic special. \textcolor{AttentionColor}{solved! Yes}
  \item Prove that for metaplectic groups, whittaker support of irreducible genuine $(\fg,K)$-modules are metaplectic quasi-admissible. \textcolor{AttentionColor}{Use arguments of Gomez-Gourevitch-Sahi}
...
\textcolor{AttentionColor}{answer: Yes!} \cite{BV82}
