...
\begin{thm}[see \cite{AdC09}]
  Inner automorphism group $\Inn(G)$ of $G$ is equal to $\mathrm{G/Z(G)}$,
  $\Inn(G)$ acts freely and transitively on the set of pinnings.
\end{thm}
...
\begin{thm}
  If $\RG$ is a compact connected group, then its inner automorphism group $\Inn(G) = \mathrm{G/Z(G)}$
...
\begin{proof}
  Here's proof from mathoverflow \href{https://mathoverflow.net/questions/377981/classification-of-not-necessarily-connected-compact-lie-groups/378141#378141}{LSpice's answer}:
  \begin{figure}[H]
    \centering
    \includegraphics[width=1.0\linewidth]{figures/pinnings-structure-theory.png}
  \end{figure}

  \begin{figure}[H]
    \centering
    \includegraphics[width=1.0\linewidth]{figures/pinnings-transitivity-proof.png}
  \end{figure}
\end{proof}
...
\begin{figure}[H]
  \centering
  \includegraphics[width=1.0\linewidth]{figures/grothendieck-butterfly.png}
...
$\iota: G \to  G_\BC$ such that its differential $\mathrm{d\iota} : \Lie(G) \to \Lie(G_\BC)$
has the following two properties:
\begin{enumerate}
  \item the kernel of $\mathrm{d\iota}$ is contained in the center of $\Lie(G)$;
  \item the image of $\mathrm{d\iota}$ is a real form of $\Lie(G)$.
\end{enumerate}
...
\begin{enumerate}
  \item $\Rep(\fg,Q_\iota)$ the category of all finite-dimensional representations of $\fg$ whose weight are contained in $Q_\fg$, \uncertain{it can be identified with $\Rep(Lie(G_\BC),Q_\iota)$ and hence $\Rep_{finite}(G_\BC)$};
  \item $\Rep(G)$ the category of all Casselman-Wallach representations of $G$, and its full subcategories $\Rep_\mu(G)$, $\Rep_S(G)$.
\end{enumerate}
...
\begin{defn}[coherent family]
  ...
  \end{enumerate}
  The set of coherent families is denoted by $\Coh_\Lambda(\mathcal{K}(G))$
\end{defn}
We have the integral Weyl group $W_{\Lambda} = \set{w \in W}{w\lambda - \lambda \in Q_\iota}$
there is a representation of $W_\Lambda$ on $\Coh_\Lambda(\mathcal{K}(G))$ by
\[(w \cdot \varPhi)(\nu) = \varPhi (w^{-1}\nu).\]
...
for it, use Langlands classification, follow \cite{BMSZ22}.

Suppose $H$ is a Cartan subgroup of $G$, by which we mean a subgroup that is the centralizer of
a Cartan subalgebra of $\Lie(G)$ \clarification{(In the case of linear groups, it's just a
...
\begin{enumerate}
  ...
  \item For every $\Gamma \in \Irr(H)$ differential of $\Gamma$ is a direct sum of one-dimensional representations attached to a unique $\mathrm{d\Gamma \in \fh^*}$.
...
\begin{defn}
  ...
  \begin{enumerate}
    ...
    \item $\mathrm{d\Gamma_{\nu}} = \xi(\nu) + \delta(\xi)$ for all $\nu \in \Lambda$.\draftnote{(note that Cartan subgroups not always have irr repn with this differential!)}
  \end{enumerate}
...
\begin{lem}[a simple discovery of abstract algebra]
...
\begin{enumerate}
  \item $\{ \textrm{split connected reductive algebraic groups} \}/_\backsim \ = \{ \textrm{root data}\} /_\backsim $;
