...
\begin{proof}
  ...
  Again, use induction, suppose that we have found $\widehat{y}_i^{(n)}$ lift $y_i$ such that $\frac{1}{\hbar}\left[\widehat{y}_i^{(n)}, \widehat{y}_j^{(n)}\right] = \omega_{ij} + \hbar^{n} R_{ij}^{(n)}$ for some $R_{ij}^{(n)} \in \CA_{\hbar}$, we want to find $\delta_i^{(n)} \in \hbar^{n+1} \CA_{\hbar}$ such that $\frac{1}{\hbar}\left[\widehat{y}_i^{(n)} + \delta_i^{(n)}, \widehat{y}_j^{(n)} + \delta_j^{(n)}\right] = \omega_{ij} + \hbar^{n+1} R_{ij}^{(n+1)}$.
