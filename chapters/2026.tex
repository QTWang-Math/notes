...
\begin{rk}
  After I have a sleep, I find that $\widetilde{G}_0$ can be repleced by $\widetilde{G}$ in this theorem, and the proof is even easier.
\end{rk}
...
\notesection{2026-03-21}{Geometry of the flag variety of \texorpdfstring{$\mathrm{SL}_2(\BC)$}{SL2(C)}}
...
The enhanced flag variety $\widetilde{\CB}$ is defined as the variety of pairs $(\fb, \{x_{\alpha}\}_{\alpha \in \Sigma})$ where $\fb$ is a Borel subalgebra of $\fg$, and $\xi$ is a nonzero element in $(\fn/[\fn,\fn])^*_{\alpha}$. There is a natural projection map $\pi : \widetilde{\CB} \to \CB$ which forgets the second component, and is an $H$ principle bundle.
...
\begin{thm}[Main theorem]
  For a nilpotent otbit $\BO$ in a complex semisimple Lie algebra $\fg$, its closure $\overline{\BO}$ is normal if and only if $\BC[\BO] = \BC[\overline{\BO}]$.
\end{thm}
...
\begin{proof}[Proof of Main Theorem]
  ($\Rightarrow$) If $\overline{\BO}$ is normal, then by Serre's criterion, we get infromation about the depth inequality, and then we can apply the algebraic Hartogs theorem \cite[Chapter III, Exercise 3.5]{Har77} to conclude that $\BC[\BO] = \BC[\overline{\BO}]$.
...
\begin{rk}
  ...
  \[
    \pi: \mathrm{Spec}(\BC[\widetilde{\BO}]) \to \overline{\BO},
  \]
  $\mathrm{Spec}(\BC[\widetilde{\BO}])$ has finitely many symplectic leaves, and we also know that $\mathrm{Spec}(\BC[\widetilde{\BO}])^{reg}$ is symplectic, it has no proper Poisson subvarieties, hence it is a symplectic leaf. By in this situation, symplectic leaves are not in one one correspondence with $G$-orbits any more, so $\widetilde{\BO}$ may properly contained in $\mathrm{Spec}(\BC[\widetilde{\BO}])^{reg}$.

  Actually in the case of exceptional Lie algebras, there exist some nilpotent orbits $\BO$ such that $\mathrm{codim}_{\overline{\BO}}(\overline{\BO} \backslash \BO) = 2$ but $\mathrm{codim}_{X}(X \backslash X^{reg}) \geq 4$, where $X = \mathrm{Spec}(\BC[\BO])$.
\end{rk}
...
Suppose $\BO = G\cdot X$. Then the Jacobson-Morosov resolution of $\overline{\BO}$ is just the Springer resolution $T^*\CB \to \overline{\BO}_{prin} = \CN$, so $\BO$ is the principle nilpotent orbit.
...
$Y$ be an algebraic variety, $H$ is an algebraic group. $\pi: \widetilde{Y} \to Y$ is a principle $H$-bundle. Then we have the following reduction:
...
\begin{prop}
  \begin{itemize}
    \item $\mathrm{Pic}(X) = 0$;
    \item $\mathrm{Pic}(X^{reg}) = X^*(\widetilde{H})$, where $\widetilde{H}$ is the stabilizer in $\widetilde{G}$ of a point in $\widetilde{\BO}$.
  \end{itemize}
\end{prop}
...
\begin{proof}
  ...
  $$\mathrm{Pic}(X^{reg}) = \mathrm{Pic}(\widetilde{\BO}).$$
  ...
  \[
    \mathrm{Pic}(\widetilde{\BO}) = \mathrm{Pic}(\widetilde{G}/\widetilde{H}) = \mathrm{Pic}^{\widetilde{G}}(\widetilde{G}/\widetilde{H}) = X^*(\widetilde{H}).
  \]
  The second equility is due to the fact that $\widetilde{G}$ is simply-connected, so any line bundle on $\widetilde{G}/\widetilde{H}$ admits a $\widetilde{G}$-equivariant structure. And $\CO_X$ has only one $\widetilde{G}$-equivariant structure, since $H^1(G;\CO^*_X(X)) = 0$.
\end{proof}
...
\begin{prop}
  $\mathrm{Pic}(Y) = \mathrm{Pic}(G/P) = X^*(\widetilde{L})$.
\end{prop}
...
\begin{thm}
  If $\widetilde{\BO_L}$ is birationally rigid, then $H^2(Y^{reg},\BC) = \mathrm{Pic}(Y) \otimes_{\BZ} \BC = \fz(\fl)$.
\end{thm}
...
When $X = \Spec(A)$ is affine, use micro-localzation, we can identify the formal quantization of $X$ with a formal quantization of $A$.
