...
For any $w \in W$, there is a unique nilpotent orbit $\mathrm{St}(w)$ such that $\mu(\overline{T^*_{\mathcal{Y}_w}(\mathcal{B} \times \mathcal{B})}) = \overline{\mathrm{St}(w)}$. The map $w \mapsto \mathrm{St}(w)$ defines a \textcolor{red}{surjective(?)} map from $W$ to the set of nilpotent orbits in $\fg^*$ (this map can be computed via Robinson-Schensted algorithm).
...
\[
  \BR p_{\CN,*}\underline{\BQ}_{\widetilde{\CN}} \simeq \BR p_{*}\underline{\BQ}_{\widetilde{\fg}})|_{\CN}. \quad  \textcolor{red}{(\mathrm{Spr} = \BR p_{\CN,*}\underline{\BQ}_{\widetilde{\CN}}[\dim \CN] \ called \ The \ Springer \ sheaf)}
\]
