...
\begin{defn}
  ...
  \begin{itemize}
    ...
    \item There is a resolution of singularity $\pi: Y \to X $ such that $\pi^{*}(\omega^{reg}) \in \Gamma(\pi^{-1}(X^{reg});\Omega_Y)$ extends to a 2-form $\tilde{\omega}$ in $\Gamma(Y;\Omega_Y)$. ($\tilde{\omega}$ is a closed $2$-form, but possibly degenerate) 
  \end{itemize}
\end{defn}
