...
\usepackage{amscd,amsthm,amsfonts,amssymb,amsmath,amsxtra}
\usepackage{mathrsfs}
...
\hypersetup{
    pdftitle={Notes},
    pdfauthor={IDLE},
    colorlinks=true,       % 关键：开启文字颜色
    linkcolor=BrickRed, % 对应图中红框部分（回引页码）的文字颜色
    citecolor=RoyalBlue, % 对应图中蓝框部分（MR编号）的文字颜色
    urlcolor=ForestGreen,  % 网页链接颜色
    pdfborder={0 0 0}       % 彻底消除所有方框
}
...
% 0 = 只显示章 (Chapters)
% 1 = 显示到节 (Sections) —— 这通常是隐藏子节最常用的设置
% 2 = 显示到子节 (Subsections)
...
\section{\texorpdfstring{2025.8.14\protect\footnote{This section is organized by Ryan Ang and Qiutong Wang at Tianyuan Mathematics Research Centre.}}{2025.8.14}}
...
% Draw equator
\draw[dashed] (-1,0) arc (180:360:1 and 0.4);
\draw (-1,0) arc (180:0:1 and 0.4);
...
\draw[red, thick] (1,0) arc (0:-180:1); % lower semicircle
\node[red, below] at (0,-1) {$K$};
...
Construuctible sheaves on $\BC = \BC^* \sqcup \{0\}$.
...
\begin{rk}
  Actually when $j: S \hookrightarrow  X$ is \colorbox{yellow!50}{affine locally closed embedding} , $j_!$ and $j_*$ are $t$-exact for the perverse $t$-structure, so $j_!\CL[\dim S]$ and $j_*\CL[\dim S]$ are perverse sheaves on $X$.
\end{rk}
...
\[
  (\fn/[\fn,\fn])^* = \bigoplus_{\alpha \in \Sigma} (\fn/[\fn,\fn])^*_{\alpha},
\]
...
\]
which sends $(u,v)$ to $u_{Y} + v$, where $v \in V$ is regarded as a tangent vector of the fiber $V$ at $\alpha$, and $u_{Y}$ is the vector field on $Y$ generated by the action of $u \in \fu \subset \fg$.
...
The above calculation is instrumental in extending the pullback of the Kostant-Kirillov symplectic form to the Jacobson-Morozov resolution of a nilpotent orbit:
\[
\begin{tikzcd}
...
\bibitem[BB93]{BB93}
A.~A. Beilinson and J.~N. Bernstein, A proof of Jantzen conjectures, in {\it I. M. Gelfand Seminar}, 1--50, Adv. Soviet Math., 16, Part 1, Amer. Math. Soc., Providence, RI, ; \MRR{1237825}
